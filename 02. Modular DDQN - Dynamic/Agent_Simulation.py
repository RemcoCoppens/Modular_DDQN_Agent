...
class AGENT_Simulation:
    def __init__(self, simTime, epsilon=1):
        ...
        self.track_arrival_rate = []
        self.track_mfg_rate = []
        self.track_stp_rate = []
        arr_rate, self.arrival_dist = self.set_dist(unif=GC.arrival_unif)
        mfg_rate, self.mfg_dist = self.set_dist(unif=GC.process_unif, nr_of_workstations=self.nr_of_workstations)
        stp_rate, self.setup_dist = self.set_dist(unif=GC.process_unif, nr_of_workstations=self.nr_of_workstations)
        self.track_arrival_rate.append(arr_rate)
        self.track_mfg_rate.append(mfg_rate)
        self.track_stp_rate.append(stp_rate)
    ...
    def set_dist(self, unif, nr_of_workstations=None):
        """ Return distribution to pull stochastic variables in simulation """
        # If variable is not specifically for workstations
        if nr_of_workstations == None:
            rate = unif.rvs()
            return rate, {p: Distribution(stats.expon(scale=1/rate)) for (i, p) in enumerate(GC.product_list)}
        else:
            rates = [unif[p].rvs() for p in GC.product_list]
            return rates, {p: [Distribution(stats.expon(scale=1/rates[idx])) for i in range(nr_of_workstations)] for (idx, p) in enumerate(GC.product_list)}
    ...
    def reset(self):
        ...
        # Reset environmental distribution
        arr_rate, self.arrival_dist = self.set_dist(unif=GC.arrival_unif)
        mfg_rate, self.mfg_dist = self.set_dist(unif=GC.process_unif, nr_of_workstations=self.nr_of_workstations)
        stp_rate, self.setup_dist = self.set_dist(unif=GC.process_unif, nr_of_workstations=self.nr_of_workstations)
        self.track_arrival_rate.append(arr_rate)
    ...
    def run_episodes(self, nr_of_episodes, save_models=True, learn_agent=True, return_agg=False):
        """ Run multiple episodes and return results. """
        # Loop over the number of episodes desired
        for i in range(1, nr_of_episodes+1):
            ...
            max_perf = max(self.performance) if len(self.performance) > 0 else 0
        ...
        if return_agg:
            ...
            SE_performance = np.std(self.performance) / np.std(nr_of_episodes)
            self.performance = []
