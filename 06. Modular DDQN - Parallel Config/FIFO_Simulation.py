...
class FIFO_Simulation:
    ...
    def run_sim(self):
        ...
        while time < self.simulation_time: 
            ...
            late = [order for order in self.queue if order.due_date < time]
            for late_order in late:
                self.queue.remove(late_order)
            ...
            if event.type == GC.OrderArr:
                ...
                # Pull destination index from order steps
                _ = order.steps.pop(0)
                
                # Retrieve list of available workstations
                unoccupied = [ws for ws in self.WS if ws.occupied == False]
                
                # If there is a workstation idle
                if len(unoccupied) > 0:
                    # Retrieve workstation object of the first in the list
                    destination = unoccupied[0]
                    
                    # Create start production event for the selected workstation
